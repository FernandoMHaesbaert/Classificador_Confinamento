...
bootstrap_firth <- function(
      ...
      if(!inherits(modelo_firth, "logistf")) {
            stop("O objeto informado não é um modelo logistf.")
      }
      # ===================================================
      ...
      resumo_coef <- purrr::map_dfr(
            ...
            function(v) {
                  ...
                  tibble::tibble(
                        ...
                        DP = sd(valores),
                        IC_inf = as.numeric(quantile(valores, probs = alpha_inf)),
                        IC_sup = as.numeric(quantile(valores, probs = alpha_sup)),
                        Prob_coef_positivo = mean(valores > 0),
                        Prob_coef_negativo = mean(valores < 0))
            }
      )
      ...
      resumo_or <- purrr::map_dfr(
            ...
            function(v) {
                  ...
                  tibble::tibble(
                        Variavel = v,
                        OR_medio = mean(valores),
                        OR_mediano = median(valores),
                        DP = sd(valores),
                        IC_inf = quantile(valores, probs = alpha_inf),
                        IC_sup = quantile(valores, probs = alpha_sup),
                        Prob_OR_maior_1 = mean(valores > 1),
                        Prob_OR_menor_1 = mean(valores < 1))
            }
      )
      ...
      estabilidade <- resumo_or |>
            dplyr::mutate(
                  Estabilidade = dplyr::case_when(
                        Prob_OR_maior_1 >= 0.95 | Prob_OR_menor_1 >= 0.95 ~ "Muito alta",
                        Prob_OR_maior_1 >= 0.80 | Prob_OR_menor_1 >= 0.80 ~ "Alta",
                        Prob_OR_maior_1 >= 0.65 | Prob_OR_menor_1 >= 0.65 ~ "Moderada",
                        TRUE ~ "Baixa"))
      ...
      message(paste("Simulações bootstrap:", n_boot))
      message(paste("Variáveis avaliadas:", ncol(beta_boot_sem_intercepto)))
      ...
      # RETORNO
      # ===================================================
      return(list(resultados = resultados,
                  resumo_coef = resumo_coef,
                  resumo_or = resumo_or,
                  estabilidade = estabilidade,
                  beta_boot = beta_boot_sem_intercepto,
                  or_boot = or_boot,
                  n_boot = n_boot,
                  nivel_ic = nivel_ic))
}